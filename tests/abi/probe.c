/* The ABI probe in C. For every struct and union it prints the size and
   the alignment. For every field it prints the bytes of a value with only
   that field set to 1. probe.anti prints the same for the same types, so
   any difference in layout shows as a difference in the output. */
#include <stddef.h>
#include <stdint.h>
#include <stdio.h>
#include <string.h>
