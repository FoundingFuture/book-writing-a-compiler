...
static void run(struct lowered *l, const char *source)
{
    ...
    if (!lex(source, strlen(source), &l->arena, &l->diags, &l->tokens) ||
        ...
        !sema_check(l->module, "main", NULL, 0, &l->types, &l->arena,
                    &l->diags)) {
    ...
    l->ok = lower_module(l->module, "main", &l->ir, &l->diags);
}
...
void test_lower(void)
{
    ...
    /* A str constant becomes the same global as a literal. */
    ...
    /* A constant and a literal argument get slots of their own, and a call
       with an aggregate result gives the address of the result. */
    lowers("struct V { x: f64, y: f64 }\n"
           ...
           "    %3 = slot main.V\n"
           "    store f64 1, %1\n"
           "    %2 = ptradd %1, offset_of main.V.y\n"
           "    store f64 1, %2\n"
           "    store f64 3, %3\n"
           "    %4 = ptradd %3, offset_of main.V.y\n"
           "    store f64 4, %4\n"
           "    %5 = call agg @add(%1, %3)\n"
           "    memcopy %0, %5, main.V\n"
           "    %6 = ptradd %0, offset_of main.V.y\n"
           "    %7 = load f64 %6\n"
           "    ret f64 %7\n"
           "}\n");
