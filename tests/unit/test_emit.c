                     ...
                     const char *expected)
{
    ...
    if (!lex(source, strlen(source), &arena, &diags, &tokens) ||
        ...
        !sema_check(module, "main", NULL, 0, &types, &arena, &diags) ||
        !lower_module(module, "main", &ir, &diags)) {
        check_failures++;
...
/* No Anti expression puts an address into global data, so the emitter
   refuses a relocation rather than choose a writable section for it. */
static void data_relocation(void)
{
    ...
    CHECK(!emit_program(&out, TARGET_LINUX_ARM64, &m, functions, "main",
                        error, sizeof error));
    CHECK_STR(error, "global data `main.0` holds an address, which antic "
                     "does not emit");
    text_free(&out);
    ir_module_free(&m);
    arena_free(&arena);
