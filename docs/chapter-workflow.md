...
7. Commit and push to `origin/main`. No pull request.
8. Until the first public release, rebuild the chapter commits with `tools/scripts/chapters.py` and push `main` and the tags with `--force`.
...
- The last step of the script checks out every tag in order, builds it and runs its tests. A tag that fails is fixed on `main` before the push.
