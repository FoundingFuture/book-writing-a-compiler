...
/* DESIGN: a struct that declares an abstract function is never a
   complete value. It appears behind a pointer and as an `inherits`
   field, and a literal of it only as the `inherits` part of a derived
   literal. Its own `concrete` functions do not exist. */
static bool refuse_abstract_value(struct checker *c, struct pos pos,
                                  const char *what, const struct type *t)
{
    if (t == NULL || t->kind != TYPE_STRUCT || !t->has_abstract) {
        return false;
    }
    error_at(c, pos, "%s has type `%s`, which declares an abstract function "
             "and is reached through a pointer", what, tn((struct type *)t));
    return true;
}

/* The type that t nests with `inherits`, or NULL. */
static const struct type *inherited(const struct type *t)
{
    if (t == NULL || t->kind != TYPE_STRUCT || t->field_count == 0 ||
        t->fields[0].form != FIELD_INHERITS) {
        return NULL;
    }
                                                  ...
                                                  bool *ambiguous)
{
    ...
    for (i = 0; s != NULL && i < s->field_count; i++) {
        ...
        if (f->form == FIELD_PLAIN || !provides(c, f->type, name)) {
            continue;
        }
        if (found != NULL) {
            *ambiguous = true;
...
/* DESIGN: a function of a body without `pub` is visible only to the
   functions declared in the same struct, on any instance. The checker
   compares the struct that owns the current function with the one that
   declares the member. */
static bool member_visible(const struct checker *c, const struct type *t,
                           const struct item *m)
{
    const struct item *owner;

    if (m->pub) {
        return true;
    }
    owner = c->function != NULL ? c->function->owner : NULL;
    return owner != NULL && owner->symbol != NULL &&
           owner->symbol->type == t;
}
...
static bool method_call(struct checker *c, struct expr *call)
{
    ...
    if (f == NULL || (f->kind != SYMBOL_FN && f->kind != SYMBOL_EXTERN_FN) ||
        ...
        if (hidden != NULL && !member_visible(c, s, hidden)) {
            error_at(c, field->pos, "`%.*s` is private to `%s`",
                     (int)field->as.field.name.length,
                     field->as.field.name.text, tn(s));
            return false;
        }
...
/* The function or constant that the body of t declares under name. */
static struct item *find_member(const struct type *t, const struct name *name)
{
    size_t i;

    for (i = 0; t != NULL && i < t->member_count; i++) {
...
static enum symbol_kind item_symbol_kind(enum item_kind kind)
{
    ...
    case ITEM_STRUCT:
    case ITEM_UNION:
    case ITEM_ENUM: return SYMBOL_STRUCT;
                ...
                struct arena *arena, struct diagnostics *diags)
{
    ...
    for (i = 0; i < module->item_count; i++) {
        ...
        /* DESIGN: `inherits` nests a base whole at offset 0, so it is
           the first field and there is at most one. The struct may not
           redeclare a name that the base or its chain already has. */
        for (j = 0; j < it->param_count; j++) {
            if (fields[j].form != FIELD_INHERITS) {
                continue;
            }
            if (j != 0) {
                error_at(&c, fields[j].pos,
                         "`inherits` must be the first field of `%.*s`",
                         (int)it->name.length, it->name.text);
            }
        }
    ...
    for (i = 0; i < module->item_count; i++) {
        ...
        for (j = 0; j < it->member_count; j++) {
            ...
            sym->kind = m->kind == ITEM_CONST ? SYMBOL_CONST : SYMBOL_FN;
            sym->name.text = text;
    ...
    /* A plain or `use` field of an abstract struct is refused here,
       after every struct knows whether a contract reaches it. */
    for (i = 0; i < module->item_count; i++) {
        struct item *it = module->items[i];
        ...
        for (j = 0; j < t->field_count; j++) {
            char what[96];
            if (t->fields[j].form == FIELD_INHERITS) {
                continue;
            }
    ...
    for (i = 0; i < module->item_count; i++) {
        ...
        /* Every abstract function of the chain needs a concrete one at or
           below the struct that declares it. */
        for (base = inherited(t); base != NULL; base = inherited(base)) {
