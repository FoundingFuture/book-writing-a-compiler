...
static bool is_local_struct(const struct writer *w, const struct type *t)
{
    ...
    return t->kind == TYPE_STRUCT && t->module.length == strlen(module) &&
...
static void put_type(struct writer *w, const struct type *t)
{
    ...
    case TYPE_STRUCT:
        ...
        if (is_local_struct(w, t)) {
            put_u8(w, (uint8_t)(t->is_union | t->packed << 1));
            put_u64(w, t->align);
...
{
    const struct interface *lib = library(r, module);
    size_t i;
...
/* Move the addresses inside a constant to the indices of the program. */
