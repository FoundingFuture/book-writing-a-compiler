...
static void dump_params(struct dumper *d, const char *label,
                        ...
                        const struct type *owner)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        dump_doc(d, 2, "doc", &params[i].doc);
        dump_doc(d, 2, "note", &params[i].note);
        dump_type(d, 2, params[i].type);
        if (params[i].bits != NULL) {
            simple(d, 2, "bits", NULL);
            dump_expr(d, 3, params[i].bits);
        }
    }
}
...
static void dump_module(struct dumper *d, const struct module *module)
{
    ...
    for (i = 0; i < module->item_count; i++) {
        ...
        case ITEM_FN:
        case ITEM_EXTERN_FN:
            dump_params(d, "param", it->params, it->param_count, NULL);
            if (it->variadic) {
        ...
        case ITEM_STRUCT:
        case ITEM_UNION:
            dump_params(d, "field", it->params, it->param_count, type);
            break;
        case ITEM_CONST:
