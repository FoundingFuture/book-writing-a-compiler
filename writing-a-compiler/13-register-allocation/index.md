...
A spill inside a loop pays its store and its load on every pass of that loop. A spill outside one pays them once. The allocator gives every interval a weight and lets the cheapest interval give up its register.

The weight counts each use and each definition of the virtual register. A use inside one loop counts ten times, inside two loops a hundred times. Lowering measures the depth, because it is the pass that builds the loops. The field `loop_depth` of `struct lowerer` holds the number of loops around the statement being lowered, and a block takes that number when it is created.
...
Both `lower_loop` and `lower_for` raise the field before the body and lower it after it. The test block, the body and the step of one loop therefore carry the same depth. Instruction selection copies the number from the IR block to the machine block. The function `build_intervals` turns it into the cost of one use.
...
The bound of a hundred million stops the product at eight nested loops. A `uint64_t` holds more, but an interval that reaches the bound already outranks every interval the scan compares it with.

A constant is the one value that never needs a slot. Its definition reads nothing, so the same instruction may be written again at any point of the function. Writing it again costs one instruction, where a spill costs a store, a stack slot and a load. The function `defines_constant` recognises such a definition.
...
An instruction qualifies when it writes one virtual register and every other operand is an immediate. The function `rematerialise_constants` collects the one definition of each such register, gives every use a fresh virtual register defined just before it, and drops the original. A register with two definitions is left alone, since the value at a use is then not known from one instruction. The pass runs before liveness, so the scan sees ranges one instruction long and never considers spilling one.
...
The test `listing_spillweight.linux-x86_64.alloc` holds eleven values across a call to an external `use12` of twelve arguments. A loop runs between the definitions and the call. Every pass of the loop reads the counter. The eleven values are read once, after the loop.
...
pub fn hot(n: int) -> int
{
	...
	return total + use12(a, b, c, d, e, f, g, h, i, j, k, counter);
}
```

The counter reaches a weight of 41. One definition outside the loop counts once. The comparison in the test block and the three references in the body count ten each. The value `a` reaches 2, and `total` reaches 21. On linux-x86_64 the counter keeps `rax` across the loop. The values `a` and `total` take the slots at `56(%rsp)` and `48(%rsp)`.
...
The loop body holds no load or store of the counter. The two zeros that start `total` and `counter` are the rematerialisation: the selector wrote one definition of the constant 0, and the pass gave each use its own.
...
The allocated code of this chapter is compared on two ARM64 and two x86_64 targets, and every listing above is pinned. The build has zero warnings, and every `ctest` test passes.
