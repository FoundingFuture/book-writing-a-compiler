...
The listing was checked on an Apple Silicon Mac with macOS 26.6.2. llvm-mc from the LLVM 23.1.1 release archive assembled it without a warning. The Apple linker `ld`, version `ld-1267`, linked it with `libanti_rt.a`, the runtime library built from `rt/` in the book repository. The program exits with code 42.
