...
A byte string `b"..."` has type `[]byte` and takes the same path. Its bytes may include 0, and chapter 2 makes a write through its slice undefined behaviour. Equal bytes give one global, so `"hi"` and `b"hi"` share storage. A constant of type `str` is stored like its literal, in a slot filled at each use, with the address of the same global.
...
The search in `literal_address` skips the globals of other modules. The unit test `keeps_literals` in `tests/unit/test_modules.c` loads a library `words` whose function returns `"hi"`, and its `main` module uses `"hi"` too. The IR of the program holds two globals with equal bytes.
...
The emitter writes the globals after the functions, into the read-only data section of each object format. Each global is a label, the alignment when it exceeds 1 byte, and lines of at most 16 bytes.
                      ...
                      const struct ir_module *m)
{
    ...
    text_appendf(out, "    .section %s\n",
                 data_sections[target_info(t)->format]);
...
The unit tests in `tests/unit/test_lower.c` check literals and their shared globals, byte strings, indexing, slicing of a `str` and of an array, slice literals and `str` constants. The file `tests/unit/test_emit.c` checks the data sections of ELF, Mach-O and COFF and the refusal of relocations. The file `tests/unit/test_modules.c` checks the literals of an imported library, and `tests/unit/test_utf.c` the runtime conversions. The dump tests pin the IR of `strings.anti` and its assembly files for three targets, and the asm tests assemble it for all six triples. The build has zero warnings, and every `ctest` test passes.
