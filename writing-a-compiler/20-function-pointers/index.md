...
The unit tests in `tests/unit/test_lower.c` check function values, calls through parameters, locals and fields, and shared signatures with extensions. The files `tests/unit/test_x86_64.c` and `tests/unit/test_arm64.c` check indirect calls and C function addresses on Linux and Windows, and `tests/unit/test_emit.c` the Mach-O forms. The file `tests/unit/test_modules.c` checks a signature in a library file, and `tests/unit/test_ir.c` the verifier. The dump tests pin the IR of `fnptr.anti`, its listings for linux-x86_64 and linux-arm64 and its assembly file for macos-arm64. The build has zero warnings, and every `ctest` test passes.
