...
The unit tests in `tests/unit/test_select.c` compile programs through the optimizer and compare the selected code on the six targets. They cover the function `scale`, the loop above and comparisons as values. They also cover constants beyond the immediate limits, the two-operand forms and the calls of a C function. The test `widen` checks the conversion of `c_long` and `c_wchar` to `int` on both Windows targets and on linux-x86_64, with the conversion patterns of chapters 14 and 15. In `tests/unit/test_layout.c`, the test `lengths` checks the folded array length and its error. The tests `dump_select_arm64`, `dump_select_x86_64`, `dump_select_loop_arm64` and `dump_select_loop_x86_64` pin the listings of this chapter. The build has zero warnings, and every `ctest` test passes.
