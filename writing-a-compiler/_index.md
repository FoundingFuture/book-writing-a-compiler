...
The compiler builds with CMake 3.20 and a C11 compiler. Compiling an Anti program needs llvm-mc and lld of one LLVM release, 23.1.1. One CMake command installs them on every host, from an archive that Anti serves. lld links every target. The option `--linker platform` selects the linker of the operating system instead, and chapter 3 gives the command and the archives.
