...
The x86_64 float code assembles for all three x86_64 triples in the asm tests, and it does not run on the development Mac.

## Tests

The unit tests in `tests/unit/test_float.c` check arithmetic, negation, constants, memory, floats kept across calls, comparisons, conversions and a far spill on ARM64. The dump tests pin the float listings of `floats.anti` for two targets and of `floatcalls.anti` for five. The asm tests assemble both programs for all six triples. The build has zero warnings, and every `ctest` test passes.
