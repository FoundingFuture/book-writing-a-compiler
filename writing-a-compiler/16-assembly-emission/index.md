                 ...
                 bool one_module, char *error, size_t error_size)
{
    ...
    for (i = 0; i < m->global_count; i++) {
        if (m->globals[i]->reloc_count > 0) {
            snprintf(error, error_size, "global data `%s.%s` holds an address, "
                     "which antic does not emit", m->globals[i]->module,
                     m->globals[i]->name);
            return false;
        }
    }
                     ...
                     const char *object)
{
    ...
    const char *argv[] = {o->llvm_mc != NULL ? o->llvm_mc : "llvm-mc", NULL,
                          "-filetype=obj", "-o", object, assembly, NULL};
    int run;
                       ...
                       struct link_facts *f)
{
    ...
    text_appendf(&marker, "%s/%s", text_cstr(&f->sysroot),
                 ...
                 : target_info(t)->arch == ARCH_ARM64
                     ? "crt/lib/aarch64/libcmt.lib"
                     : "crt/lib/x86_64/libcmt.lib");
    ...
    if (!present && os != OS_WINDOWS) {
        fprintf(stderr, "antic: linking for %s with lld needs the sysroot "
                        "%s, which tools/get-sysroot.sh installs\n",
...
/* DESIGN: ld.lld links a Linux program statically against the musl of
   ...
   linker, so it runs on any Linux kernel. */
...
The Windows sysroot comes from xwin, a tool that downloads the Microsoft C runtime and the Windows SDK[^26]. It lays them out for a link on another system. Its directories `crt/lib/<arch>`, `sdk/lib/um/<arch>` and `sdk/lib/ucrt/<arch>` hold the libraries[^26], and lld-link receives them with `/LIBPATH`. The option `--accept-license` of xwin accepts the licence without a prompt[^26]. The script `tools/get-sysroot.sh` runs xwin only when its caller passes that option.
...
```c
/* link.exe or lld-link with the static C runtime. */
...
Every `.anti` file in `tests/programs` is compiled, linked and run on the development Mac, a macos-arm64 host. Chapter 3 had two of them, and this chapter adds ten.
...
[^24]: LLVM Project, *ld.lld(1)* of release 23.1.1, options `--static`, `--pie` and `--no-dynamic-linker`, https://github.com/llvm/llvm-project/blob/llvmorg-23.1.1/lld/docs/ld.lld.1
