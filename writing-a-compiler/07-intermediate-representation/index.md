...
An `assert` reaches the IR as a branch, not as an instruction of its own.

```
    branch %2, b2, b1
b1:
    %3 = addr @main.0
    call void @anti_rt_assert_failed(%3, 34)
    jump b2
b2:
```

The failure block carries a flag, `assert_fail` on `struct ir_block`. That flag is the whole mechanism. It marks the arm that a build without assertions cuts. It also survives into a library file, so an assertion written in one module follows the build that compiles the program using it.

A build that drops assertions replaces the branch with a jump to the block that follows. The failure block then has no predecessor, and the pass that removes unreachable blocks takes it, the call and the text with it. Nothing in the back end knows that an assertion existed.

The text of a failure is built during lowering and lives in read-only data. It holds the file, the line and the source of the condition, so the runtime routine prints one string and aborts.

### The error branch

The error forms of chapter 2 reach the IR the same way, with no instruction of their own. A function that can fail returns a pointer, and a null pointer is success. The call is therefore followed by one `eq` against 0 and one `branch`, and the arms are the block after the call and the block of the handler.

```text
    %1 = call ptr @main.parse_int(%0)
    %2 = eq i8 %1, 0
    branch %2, b2, b1
```

`catch fatal` calls the entry of `fatal` in the table of the error's class. `try` runs the deferred statements of the function and returns the pointer. A `catch` block runs its statements, and `yield v` writes `v` where the call would have written its result. A `try` block gives one handler to every failing call of the block, so each of them branches to the same handler.

Nothing unwinds and nothing crosses a function boundary, which is why the whole of it is one compare and one branch.
...
The IR has no class. A class reaches it as an aggregate whose field 0 is the base. The chain is therefore a nesting of ordinary aggregates, and the root holds one pointer. The tables, the descriptors and the ancestor arrays are globals with constant values, which chapter 8 builds.

```text
type anti.rt.Object = struct { table: ptr }
type classes.Shape = struct { super: anti.rt.Object, x: f32, y: f32 }
global classes.Shape.table [10]ptr { @classes.Shape.descriptor, ... }
```

A table holds addresses of functions, so a constant gains a kind for one. `IR_CONST_FUNC` names a function by index, as `IR_CONST_ADDR` names a global. The back end writes a relocation for either, and the relocation records which of the two its target is.

A dispatched call is a load of the table from the object, a load of the entry, and an indirect call. The index of the entry is a constant, and its offset stays symbolic, because the IR holds no sizes.
...
An atomic operation is a call of the runtime. It takes the address, the width of the value as a symbolic size, and the operands. `dup`, `delete` and `destroy` are calls of the runtime as well. Each takes the pointer alone, and the runtime walks the descriptor of the object.

A static field is a global that the program writes, which no other global is. The IR flag that says so is `mutable` on `struct ir_global`, and it is not the contextual word of a singleton field. It sends the global to the writable section of the object format, where a literal and a relocated constant never go.

## Excluded from the IR

Six kinds of information never appear in the IR, because each depends on the target. The object model adds none of them.
...
The builder, the text form and the verifier are pinned against the modules of this chapter, including the three messages above. The build has zero warnings, and every `ctest` test passes.
