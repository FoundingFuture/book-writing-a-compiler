...
The unit tests in `tests/unit/test_arm64.c` check comparisons of narrow values, division, shifts, immediates, addressing modes, `adrp`, calls on three conventions, large frames and the Windows probe. The tests in `tests/unit/test_select.c` expect code for a division and a ninth parameter, which chapter 12 refused. The file `tests/unit/test_modules.c` checks the extensions in library files and rejects an extension on a 64-bit parameter. The build has zero warnings, and every `ctest` test passes.
