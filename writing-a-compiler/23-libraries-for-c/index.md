...
[Chapter 22, Threads]({{% relref "/programming/writing-a-compiler/22-threads" %}}), adds `worker fn` and `parallel`, the thread pool of the runtime and the platform layer under it. It gives the pointer-free rule that keeps a worker away from memory another worker writes.
...
With `--bundle-runtime` the archive holds the runtime, for a C project that wants one file. The function `bundle` lists the members of the runtime library with `llvm-ar t` and extracts each one with `llvm-ar x` into the directory `<base>.rt`[^4]. It leaves out the object of `rt/start.c`, whose C `main` belongs to executables. It also leaves out the object of `rt/license.c`, whose text reads the notice `anti_licenses` that a static library does not have. In its place the bundle takes `anti_rt_license_stub` from `lib/<target>/` of the runtime archive, whose `anti_rt_license_text` returns an empty text. A library that calls `anti.license.text` therefore links, and its notice comes from the package header that `anti license --from-archive` reads. The stub lies beside the runtime library and never inside it, so a program links the real text. The test `clib_bundle` builds such a library and runs a C program that prints the length 0. On ELF and Mach-O it joins the module object and the runtime objects into one relocatable object `<base>.bundled.o`. COFF has no relocatable link, so a Windows archive holds the runtime objects as members of their own.
...
## The C view of a class

A struct crosses to C as the struct C would have written. A class crosses as the same
bytes with more declarations. They are the nested layout, one table type per class and
per interface, the tables and descriptors themselves, and the functions that reach
them.
...
`antic --lib static` on that module writes `canvas.h`. The root of every chain comes
first, once per header, with the record at entry 0 of every table.
...
Then the table type of each class and of each interface, which holds the descriptor and
one slot per public function of that chain. The seven functions of the root take and give Anti values, so
the header gives C their slots and not their signatures.
...
Then the layout, with the base nested and the table pointer innermost.
...
C lays both out as antic did, because the rules are the C rules of chapter 18. A C
program may therefore hold a `Square`, take the address of its `base` and pass that
where a `Shape *` belongs.

### The symbols

The header then declares the data and the functions.
...
Each interface the class implements adds a table of its own and the helper that
reaches the sub-object. The helper takes its address, which is the pointer the
interface expects.
...
Every public function of an export class has the C symbol `Class_fn`, with `self`
written out as the first parameter. A dot is no C identifier, so `c_symbol` in
`src/target.c` writes the dot of the name `Square.area` as `_`.

```c
int32_t Square_area(Square *self);
```

Beside each one the header writes a `static inline` wrapper under the `anti_` prefix
that reads the table of the object. The direct call runs one body. The wrapper runs
the body the concrete class filled.
...
`anti_<Class>_init` is the one helper the compiler generates rather than declares.
Lowering writes it for every export class. It stores the table pointer of the class
and of every interface sub-object. It then writes the default of every field of the
chain and runs the `construct` that takes no arguments. A C program allocates the object however
it likes and calls `init` on it.

A class whose `construct` takes arguments gets `anti_<Class>_construct(c, args...)`
beside it, which returns the `struct anti_Error *` that the `construct` gives.

`anti_<Class>_delete`, `_destroy` and `_dup` are `static inline` wrappers around the
runtime, which reads the descriptor of the object and needs no generated code per
class.

An abstract class has no complete value, so it gets the layout and the table type and
neither a table symbol nor an `init`. The header says so in a comment where the two
would stand.

### A C program with Anti classes

The test `clib_classes` builds the module above as a static library and links this
program against it. The same file compiles as C11 and as C++17.
...
It prints `3 5 16`, then `16 0`, then `7`. The first line is the two coordinates after
`Shape_move` and the area through the table, which reaches `Square.area` because the
object is a `Square`. The second line is the same area called directly and then
`Shape_area`, whose own body gives 0. That pair is the whole difference between a
direct call and a call through the table, in one line of C.

The third line goes through the interface. `anti_Square_as_Ink` gives the address of
the sub-object, and `anti_Ink_colour` reads the table that sits there. The entries of
that table are thunks, so the call lands on `Square.colour` with the object and not
with the sub-object.

### An Anti class that wraps a C struct

The other direction needs no new machinery. A binding declares the C struct and its
functions, and a class holds one and owns it.
...
class TextFile
{
	...
	{
		return fgets(into.ptr, into.len as c_int, self.handle) != null;
	}
...
The `destruct` body hands the handle back to the C library, and `delete` runs it before it
frees the object. It then sets the field to null, so the free of the `own` field has
nothing left to do. The `own` marker says the class owns what `handle` points at,
which is what stops the default `copy` from sharing one file between two objects.

A C library that hands out a pointer and takes it back is the shape the `destruct` chain
was built for. The program is `tests/programs/wrap_c.anti`, and it prints `1 one
line`.
...
A C thread that calls an exported function is the dispatching thread for every `parallel` inside it. [Chapter 22, Threads]({{% relref "/programming/writing-a-compiler/22-threads" %}}), defines `parallel` and the worker pool. Every exported function of a library shares one pool. A C program that calls exported functions from two threads at once runs two `parallel` blocks on that pool, and a saturated pool runs jobs inline. Anti holds mutable global state as atomics alone, and a singleton may carry `mutable` fields that a library documents. An exported function is otherwise reentrant, unless its `///` comment says otherwise.
...
[Chapter 25, Where to take Anti next]({{% relref "/programming/writing-a-compiler/25-where-to-take-anti-next" %}}), closes the book. It gives the shape of the ideas the compiler leaves open, from SSA to the optimisations that follow it. It also says where the tools and the libraries live.
