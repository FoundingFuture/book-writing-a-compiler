...
A test fails when llvm-mc exits with an error or prints a message. The programs `main`, `loop`, `twice`, `cell`, `spill7`, `sum`, `args` and `x86` of this chapter assemble for both triples. On the development Mac, x86_64 binaries hang at start under Rosetta, so these tests assemble the code without running it. Chapter 21 runs the test programs on all six targets.

## Tests

The unit tests in `tests/unit/test_x86_64.c` check division, unsigned shifts, 8-bit arithmetic, the probe of a Windows frame and `lea` relative to `rip`. The tests in `tests/unit/test_select.c` expect code for a division and a fifth Windows parameter, which chapter 12 refused. The dump tests pin the listings of `sum`, `args` on both conventions and `x86`. The build has zero warnings, and every `ctest` test passes.
