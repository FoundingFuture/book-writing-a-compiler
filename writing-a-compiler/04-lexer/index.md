...
The file `src/lexer.h` defines one kind per keyword, one per operator, one per group of literals and one per doc comment marker. The keyword `int` is `TOKEN_INT_TYPE`, and an integer literal such as `42` is `TOKEN_INT`. The twelve words that chapter 2 reserves for threads share the kind `TOKEN_RESERVED`, which no grammar rule accepts. A doc comment token keeps its text in `value.text`, like a string literal.
...
The test file `tests/errors/lexical.anti` holds two mistakes, an unknown escape in a string and a character literal with two characters.
...
A block doc comment holds its text on the lines between the opener and the line of `*/`. The line of the opener holds only whitespace after the marker, and the last line holds only whitespace before `*/`. Text in either place is a lexical error.

| Source | Message | Position |
|---|---|---|
| `/** text` on the first line | `` doc comment text starts on the line after `/**` `` | the opener |
| `text */` on the last line | `` doc comment text ends on the line before `*/` `` | column 1 of that line |

Each line then holds text or a delimiter, and the block form gives the text of the line form. A block doc comment without `*/` reports only `unterminated block comment` at its opener, even with text on the first line. Block doc comments do not nest either, so their text cannot contain `*/`. Text with `*/` needs the line form.

```c
/* DESIGN: block doc text starts on the line after the opener. It ends on
...
The unit tests lex both forms of one comment. The line form is three `///` lines.
...
The three lines form one token at 1:1, and the next token is `fn` at 4:1. The test `dump_tokens_docs` compares this output byte for byte with `tests/dump/docs.tokens`.

## Keywords and operators

A table indexed by token kind holds the spelling of every keyword and symbol. Keyword lookup, symbol matching and the names in error messages all read that one table, so a spelling exists in one place. The table starts with 17 keywords, from `as` to `union`.
...
The table holds 54 keywords. The words `packed` and `align` have no entry and lex as identifiers. The parser of chapter 5 gives these contextual words their meaning in two positions of a struct or union declaration.
...
The test `dump_tokens_scale` compares this output byte for byte with `tests/dump/scale.tokens`. The listing in chapter 1 is a copy of that file.

## Direct path on tokens

In chapter 3 the route from source to assembly scanned characters. From this chapter on it reads the token list, which removes its own handling of whitespace and digits. The file `direct.c` of this version lives in `direct-path/` of this chapter's folder. The program `direct_path_04` builds it with the driver of chapter 3 and the lexer of `src/`, and the tests `direct_path_04_*` repeat those of chapter 3. It checks each expected token in turn and stops at the first mismatch with `expected` and the name of the missing token.
...
The minus sign and the integer literal are two tokens, and the range check uses the literal's 64-bit value. The error messages keep their positions, so the tests of chapter 3 pass unchanged.

## Tests

The unit tests for the lexer are in `tests/unit/test_lexer.c`. A test helper lexes a string and compares the token kinds with an expected list. Seven such lists cover a whole function, every symbol and every keyword, both comment forms, a byte order mark and the range `0..n`. Further tests check 7 integer, 5 float, 6 character and 13 string literals with their decoded values. They also check 33 error messages with their positions.
...
The helper `docs` lexes a source text and writes the kind and the text of every doc token, with `|` for a line feed. Five cases cover the markers, both forms, the common indentation and the ordinary openers.
...
The build has zero warnings. The ctest tests `unit`, `dump_tokens_scale`, `dump_tokens_docs` and `error_lexical` cover the lexer and pass on the development Mac.
