...
The rules above are the whole story for a class as well. Its base is field 0 of its aggregate, and those rules place field 0 at offset 0. They round the size of the base up to its own alignment before the next field starts. The trailing padding of the base therefore belongs to the base and is not reused, which is what C does for a nested struct.

The program `tests/programs/classes.anti` declares a class with two `f32` fields and a class below it with one more.
...
`Shape` holds the table pointer of the root at offset 0, `x` at 8 and `y` at 12. Its alignment is 8, from the pointer, so its size rounds up from 16 to 16. `Circle` places `Shape` at offset 0 and `r` at 16, and its size rounds up from 20 to 24. The program prints both sizes, and `size_of` reports 16 and 24 on every target.

An inherited field is reached at the offset it has in the class that declares it. Every class of a chain starts at the same address as the object, so no step per level is needed. `offset_of Shape.x` is correct on a `Circle` as well.

C sees the same bytes. The generated header of chapter 23 declares `struct Shape { struct anti_Object base; float x; float y; }` and `struct Circle { struct Shape base; float r; }`, and a C compiler lays both out as antic did. The header declares a private field too, with a comment that says so.

`packed` and `align(N)` apply to a class as they do to a struct. Both leave the base, an interface sub-object and a `use` field as they are, because each is one field of the aggregate.

An `implements` line places a sub-object of the interface inline, at the offset its field has. The sub-object begins with the table pointer of the interface, so a pointer to it is a pointer to the interface. A class with a base and two interfaces therefore lays out as the base, then one sub-object per interface, then its own fields. C reads the same nesting.

A literal writes the table pointer of the class and of every interface sub-object. Then come the named fields and the defaults of every level it left out. It writes each at the offset of the class that declares it, and it runs `construct` last. The value is therefore complete however the literal was written. A struct has no defaults of its own, so the rule belongs to classes alone.
...
The ABI probe checks the layouts and the calling conventions of this chapter against clang. It compares the bytes of the same call from both sides. The build has zero warnings, and every `ctest` test passes.
