...
The function `sema_interface` builds the interface of a checked module. Each item is a copy of the symbol of a `pub` or `internal` item, and the copy's field `home` points to the interface. An internal item carries the mark, and the loader compares the package that built the library with the package that reads it. A symbol with a `home` belongs to another module, which lowering needs to know. A constant keeps the value that semantic analysis computed, so an importing module can use it in its own constant expressions. The field `doc` is the `//!` text of the module, and each item symbol keeps its `///` text in its own field `doc`. The field `package` is the package header of the library file, which the section on the library file format describes.
...
| Section | Contents |
|---|---|
| Version stamp | The 4 bytes `ANTL` and the format version 22 |
| Package header | Name, version, dependencies, `license`, `license_text` and `attribution` |
| Public interface | Module path, imports, module doc text, type table, and the `pub` and `internal` items with their doc text |
...
Six options of antic fill the fields. The build tool `anti` passes the values of its manifest `anti.toml`, and antic without the options writes the defaults.
...
The function `antl_header` reads these fields and stops before the type table. The driver uses it to find library files and to order them before it loads any interface. The function `antl_write_header` writes the same bytes on their own. A static library for C keeps them as the copy of its package header, which chapter 23, Libraries for C, covers.
...
| Struct, union, enum or class of another module | kind, module path, name |
| Struct, union or class of the module | kind, module path, name, flags with 1 for a union, 2 for `packed`, 4 for `abstract` and 8 for `final`, `align` as u64, field count, fields, and for a class the count of its non-private functions and one record each |
| Field | name, type, bitfield width or 0, a byte of form with the `own`, `atomic` and `mutable` bits, a byte of visibility, doc text |
| Item | kind, name, type, a marks byte with `export` in bit 0 and `internal` in bit 1, doc text, then the parameter names of a function, and `variadic` for an `extern fn` or the value of a constant |
...
Both files are 1066 bytes long with the module path `com.example.geo`. They contain four doc strings, and the text `Built for the tests.`, `Not a note in the file.` and `Private, not in the file.` appears in neither.
...
With `--strip-docs` the file of the line form is 1007 bytes long, which is 59 bytes less, the length of the four strings.
...
```text
41 4e 54 4c 16 00 00 00 0b 00 00 00 63 6f 6d 2e
...
04 01 00 00 00 00 02 00 00 00 02 04 01 00 00 00
01 04 00 00 00 00 00 00 00 00 02 04 06 00 00 00
00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 ff
ff ff ff 00 00 00 00 00 00 00 00 39 04 ff ff ff
ff 01 04 01 00 00 00 00 00 00 00 00 00 00 00 00
...
| 363 to 377 | Body | two temporaries of type `i64`, one block with no assert arm, two instructions |
| 378 to 426 | Instruction | `%1 = mul i64 %0, 6` |
| 427 to 475 | Instruction | `ret i64 %1` |

The byte 57 at offset 427 is the operation `IR_RET`. The four bytes `ff` after the type byte are `IR_NO_RESULT`, and the same bytes in a value type are `IR_NO_AGG`. The unit test `writes_format` compiles the same source as the module `scale` without header options. It compares the 308 bytes with the C array `scale_antl`, which has a comment on each field.

### Version stamp

The format stores the numeric values of C enums, such as type kinds, operations and operand kinds. Adding an operation in a later chapter changes the numbers, and an older file would then decode into different instructions. Twelve static assertions pin the last value or the count of each enum.
...
A value added to or removed from one of the enums moves its last value and stops the build until the assertion is updated. The same change raises `ANTL_VERSION` in `src/antl.h`. A reader refuses every other version with a message such as `has format version 23, and antic reads version 22`.
...
The unit test `damaged_files` reads every prefix of the 308 bytes of `scale_antl`, from 8 bytes to 307, and each read fails with a message. It changes the version to 23 and the magic, which give `has format version 23, and antic reads version 22` and `is not a library file`. Three more changes make a read fail. The first sets the result type of the signature to `agg` without an aggregate. The second puts a sign extension on the `i64` parameter, and the third puts the temporary 9 into the `mul` instruction.

## Classes in a library file

A class is written like a struct, with two more bytes per field. The first holds the form of the field with its `own`, `atomic` and `mutable` bits, and the second its visibility. The flags of the type carry `abstract` and `final` as well. Its base is the type of its field 0, so the chain follows the ordinary type references. The root resolves to the class the compiler declares, and not to a module any library file carries.

A class also carries every function of its body that is not private, because a class below may call a protected one. Each record holds the name, the function type, a marks byte of the contract with the `final` and `operator` bits, a byte of visibility and the doc text. The reader builds one item per function, with the name `T.f` that its symbol has. A call in another module then resolves and reaches the symbol the library defines.

An enum carries its underlying integer and the name and number of each value. The underlying type comes first in the type table, because an enum names it by index and a reference reaches back only.

The tables, the descriptors, the ancestor arrays, the field lists and the function lists are ordinary globals of the IR. So are the table and the descriptor of every interface a class implements, whose entries name thunks, so the file carries them with every other global. A table entry names a function, and the file lists the functions after the globals. Those addresses are therefore resolved in a second pass, once the function table is read.

An interface that named a class or an enum used to write the kind byte and nothing else, which made the file unreadable. The static assertions of the version stamp catch a change to an enum of the compiler. They do not catch a change to the shape of a record, so the version rises with either.
...
The import rules and the messages above are pinned, and so is every byte of a written library file. A round trip reads a library into a new session and writes it again, and both files hold the same bytes. The build has zero warnings, and every `ctest` test passes.
